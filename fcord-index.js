...
// ── CRITIQUE : userData = dossier Fcord pour les settings/plugins
const fcordData = path.join(app.getPath("appData"), "Fcord");
app.setPath("userData", fcordData);

// AppUserModelId unique — Windows reconnaît Fcord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Fcord sont bloqués.
        ...
        function patchWebContents(wc) {
            ...
            if (wc._fcordPatched) return;
            wc._fcordPatched = true;
            ...
            wc.setWindowOpenHandler(({ url }) => {
                ...
                console.log("[Fcord][LINK] Ouverture externe:", url);
                return { action: "deny" };
            });
            ...
            wc.on("did-create-window", (childWin) => {
                ...
                if (childWc._fcordPatched) return;
                childWc._fcordPatched = true;
                ...
                childWc.on("will-navigate", (event, url) => {
                    if (!isDiscordUrl(url)) {
                        ...
                        shell.openExternal(url).catch(() => {});
                        console.log("[Fcord][CHILD-NAV] Redirection externe:", url);
                ...
                childWc.on('did-navigate', function(_event, url) {
                    if (!isDiscordUrl(url)) {
                        ...
                        console.log('[Fcord][CHILD-DID-NAV] Redirection externe apres navigation:', url);
                        try { childWin.close(); } catch (_) {}
                    }
                });
                ...
                childWc.setWindowOpenHandler(({ url }) => {
                    ...
                    shell.openExternal(url).catch(() => {});
                    console.log("[Fcord][CHILD-LINK] Ouverture externe:", url);
                    return { action: "deny" };
                });
                ...
                childWc.on("did-finish-load", () => {
                    ...
                    if (url && url !== "about:blank" && !isDiscordUrl(url)) {
                        shell.openExternal(url).catch(() => {});
                        console.log("[Fcord][CHILD-LOAD] Fermeture et redirection:", url);
                        try { childWin.close(); } catch (_) {}
                    }
                });
            });
            ...
            wc.on("will-navigate", (event, url) => {
                ...
                if (url !== currentUrl && !isDiscordUrl(url)) {
                    event.preventDefault();
                    shell.openExternal(url).catch(() => {});
                    console.log("[Fcord][NAV] Redirection externe:", url);
                }
            });
        }
        ...
        console.log("[Fcord] Patch liens externes activé sur TOUS les webContents (avec did-create-window) ✓");
        ...
        app.once("browser-window-created", (_, win) => {
            ...
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Fcord] Module bloqué (inutile pour Fcord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Fcord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Fcord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Fcord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(fcordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Fcord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Fcord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Fcord] LevelDB check failed:", e.message); }

// Modules bundlés dans fcord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Fcord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Fcord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans fcord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Fcord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
// parent.paths = []) trouvent quand même les modules natifs Fcord.
...
console.log("[Fcord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir)) {
        ...
        if (!buildInfo.localModulesRoot) {
            ...
            console.log("[Fcord] build_info.json patché → localModulesRoot:", nativeModulesDir);
        }
    }
} catch (e) {
    console.warn("[Fcord] Impossible de patcher build_info.json:", e.message);
}
