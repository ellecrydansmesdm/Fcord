...
async function shouldAutoRestart() {
    try {
        const prefsPath = path.join(process.env.APPDATA, "Fcord", "settings", "installer-prefs.json");
...
async function deleteShims(paths) {
    ...
    for (const resPath of paths) {
        log(`Removing Fcord from: ${resPath}`);
        try {
            ...
            log(`❌ Could not remove Fcord from ${resPath}`);
            log(`❌ ${err.message}`);
            return err;
        }
    }
...
export default async function(paths) {
    try {
        ...
        lognewline("Deleting Fcord loader and restoring files...");
