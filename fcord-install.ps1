# ==============================================================================
#  Fcord — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers Fcord compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte Fcord dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          FCORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "Fcord-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers Fcord ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers Fcord depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$FcordRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "Fcord-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "fcord-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'fcord-dist.zip' introuvable dans la release $version.`n           Contactez le support Fcord."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "fcord-dist.zip"
    ...
    Write-OK "Fcord $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement Fcord.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter Fcord. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  Fcord installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer Fcord.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez fcord-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
