...
export const gitHash = process.env.FCORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
...
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "fcordplugins", "fcordplugins/_api"];
            ...
            // ─── External User Plugins (~/Documents/Fcord/userplugins/) ───────────
            // Scan and auto-create the external userplugins directory.
            const externalUserPluginsDir = join(homedir(), "Documents", "Fcord", "userplugins");
            try {
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.FCORD_REMOTE;
            if (!remote) {
