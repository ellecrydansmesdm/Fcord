...
(async () => {
    ...
    // Setup userplugins
    try {
        const userTarget = join(homedir(), "Documents", "Fcord", "userplugins");
        ...
        console.error("[Fcord] Failed to setup userplugins link", e);
    }
    ...
    if (equicordFlag) {
        dirs = ["src/fcordplugins", "src/userplugins"];
    } else if (vencordFlag) {
        dirs = ["src/plugins", "src/plugins/_core"];
    } else {
        dirs = ["src/plugins", "src/plugins/_core", "src/fcordplugins", "src/userplugins"];
    }
