...
/**
 * Initializes the Fcord system tray icon.
 ...
 */
export function initFcordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            const trayMenu = Menu.buildFromTemplate([
                ...
                { type: "separator" },
                {
                    label: "Uninstall Fcord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "fcord", "Uninstall Fcord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Fcord", "Uninstall Fcord.exe"),
                            join(process.env.PROGRAMFILES || "", "Fcord", "Uninstall Fcord.exe"),
                        ];
            ...
            console.log("[Fcord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Fcord] Failed to create tray:", e);
        }
    });
}
