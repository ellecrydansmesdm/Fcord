...
import IconsTab from "@fcordplugins/iconViewer/components/IconsTab";
import { CreateThemeTab } from "@fcordplugins/createTheme/components/CreateThemeTab";
...
const TABS: TabDef[] = [
    ...
    },
    {
        ...
        description: "Sync Fcord settings across devices",
    },
    {
        id: "language",
        label: "Language",
        ...
        description: "Change Fcord's interface language",
    },
];

// ── Modal inner ───────────────────────────────────────────────────────────────

function FcordModalInner({ onClose }: { onClose: () => void; }) {
    ...
    let TabContent = null;
    switch (activeTab) {
        case "fcord": TabContent = <VencordTab />; break;
...
function FcordModal({ modalProps }: { modalProps: ModalProps; }) {
    return (
        <ModalRoot {...modalProps} size={ModalSize.LARGE} className="nc-modal-wrapper">
            <ModalContent className="nc-modal-modal-content">
                <ErrorBoundary message="Failed to render Fcord Settings modal.">
                    <FcordModalInner onClose={modalProps.onClose} />
                </ErrorBoundary>
            </ModalContent>
        </ModalRoot>
    );
...
export function openFcordModal() {
    openModal(modalProps => <FcordModal modalProps={modalProps} />);
}
