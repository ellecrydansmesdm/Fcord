...
import { EQUIBOT_USER_ID, FCORD_BOT_USER_ID } from "@utils/constants";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Fcord. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components â€” FCord Bot (domain, Component v2 Container format)
    if (message.author.id === FCORD_BOT_USER_ID) {
