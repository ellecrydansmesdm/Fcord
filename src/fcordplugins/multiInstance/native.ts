...
function createTokenPreload(token: string): string {
    ...
    const dir = join(app.getPath("userData"), "fcord-mi-preloads");
    mkdirSync(dir, { recursive: true });

    const safeToken = JSON.stringify(token); // échappe proprement le token

    const script = `
// Fcord MultiInstance — token preload
// S'exécute dans le main world AVANT Discord
(function() {
    const TOKEN = ${safeToken};
    try {
        // Définit le token dans localStorage
        Object.defineProperty(window, '__fcord_token', { value: TOKEN, writable: false });
        ...
        console.log("[FcordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[FcordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `fcord.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:fcord-mi-${userId}`;
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Fcord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[FcordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__FCORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Fcord dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.fcord.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[FcordMI] arrangeSplit error:", e);
    }
}
