...
import { t, useTranslation } from "../autoTranslateFcord";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("fcord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("fcord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Fcord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `fcord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("fcord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    ...
    stop() {
        ...
        removeHeaderBarButton("fcord-event-logs");
