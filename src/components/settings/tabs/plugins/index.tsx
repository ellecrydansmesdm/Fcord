...
function FcordTabIcon() {
    return <img src={FCORD_TAB_ICON} alt="Fcord" style={{ width: 18, height: 18, borderRadius: 4 }} />;
}
...
const categoryOptions = [
    ...
    { label: "Fcord", value: SearchStatus.FCORD },
...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Fcord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.FCORD });
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.FCORD:
                if (!pluginMeta?.folderName?.startsWith("src/fcordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (pluginMeta?.folderName?.startsWith("src/fcordplugins/") || pluginMeta?.folderName?.startsWith("src/plugins/_")) return false;
    ...
    const { fcordData, othersData, requiredData } = useMemo(() => {
        const fcordData: typeof sortedPlugins = [];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                ...
                if (folderName.startsWith("src/fcordplugins/")) {
                    fcordData.push(p);
                } else {
                    othersData.push(p);
                }
        ...
        return { fcordData, othersData, requiredData };
    }, [sortedPlugins, searchValue, newPluginsSet, depMap, pluginFilter]);

    const allDataLength = fcordData.length + othersData.length;
    ...
    const fcordVisibleData = fcordData.slice(0, Math.min(visibleCount, fcordData.length));
    const othersVisibleData = othersData.slice(0, Math.max(0, visibleCount - fcordData.length));
    ...
    const makeRequiredCard = (p: typeof sortedPlugins[number]) => {
        const tooltipText = p.required || !depMap[p.name]
            ? "This plugin is required for Fcord to function."
    ...
    const fcordPlugins = fcordVisibleData.map(makeCard);
    const othersVisible = othersVisibleData.map(makeCard);
    const requiredPlugins = requiredData.map(makeRequiredCard);

    const totalFcordPlugins = React.useMemo(() => {
        return Object.values(Plugins).filter(p => PluginMeta[p.name]?.folderName?.startsWith("src/fcordplugins/")).length;
    }, []);

    // Category-aware stats for the "ENABLED PLUGINS" card: reflects whichever tab
    // (FCORD / OTHERS / all) is currently selected, instead of always being global.
    const categoryStats = useMemo(() => {
        const isApiPlugin = (plugin: string) => plugin.endsWith("API") || Plugins[plugin].required;
        const isFcordPlugin = (p: string) => PluginMeta[p]?.folderName?.startsWith("src/fcordplugins/");

        let plugins = Object.keys(Plugins).filter(p => !isApiPlugin(p) && !Plugins[p].hidden);

        if (searchValue.status === SearchStatus.FCORD) {
            plugins = plugins.filter(isFcordPlugin);
        } else if (searchValue.status === SearchStatus.OTHERS) {
            plugins = plugins.filter(p => !isFcordPlugin(p));
        }
        ...
        <SettingsTab>
                ...
                {!premiumOnly && (
                    <div className={cl("ecosystem-banner")}>
                        <div className={cl("ecosystem-banner-text")}>
                            ...
                            <Paragraph>{t("Manage your Fcord and community plugins here. Enable, disable, and configure them to your liking.")}</Paragraph>
                        </div>
                    ...
                    <div className={cl("stats-banner")}>
                            ...
                            onClick={() => {
                                ...
                                if (native?.process?.env) {
                                    ...
                                    if (home) {
                                        ...
                                        const folderPath = isWindows ? `${home}\\Documents\\Fcord\\userplugins` : `${home}/Documents/Fcord/userplugins`;
                        ...
                        >
                            <div className={cl("stat-title")}>
                                {searchValue.status === SearchStatus.USER_PLUGINS ? t("USER PLUGINS") : t("FCORD PLUGINS")}
                            </div>
                            <div className={cl("stat-value")}>
                                {searchValue.status === SearchStatus.USER_PLUGINS ? totalUserPlugins : totalFcordPlugins}
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className={classes(Margins.bottom20, cl("filter-controls"))}>
                    ...
                    <div className={cl("filter-buttons")} style={{ minWidth: 220 }}>
                        <SearchableSelect
                            ...
                            value={categoryOptions.find(o => o.value === searchValue.status)?.value ?? SearchStatus.FCORD}
                            onChange={(v: any) => {
                            ...
                            renderOptionPrefix={(o: any) => {
                                if (o?.value === SearchStatus.FCORD) return <FcordTabIcon />;
            ...
            {premiumOnly ? (
                <>
                    ...
                    {fcordData.length || othersData.length
                        ? (
                            <div className={cl("grid")}>
                                {[...fcordPlugins, ...othersVisible].length
                                    ? [...fcordPlugins, ...othersVisible]
                ...
                <>
                    {fcordData.length > 0 && searchValue.status === SearchStatus.FCORD && (
                        <div className={cl("grid")}>
                            {fcordPlugins}
                        </div>
                    )}
                    ...
                    {searchValue.status === SearchStatus.USER_PLUGINS && (
                        <>
                            ...
                            {fcordPlugins.length > 0 || othersVisible.length > 0 ? (
                                <div className={cl("grid")}>
                                    {[...fcordPlugins, ...othersVisible]}
                                </div>
                            ) : (
                                <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)" }}>
                                    ...
                                    <div style={{ fontSize: 13 }}>{t("Add .tsx files to your")} <code>Documents/Fcord/userplugins/</code> {t("folder and rebuild.")}</div>
                                </div>
                            )}
                        </>
                    )}

                    {(searchValue.status !== SearchStatus.FCORD && searchValue.status !== SearchStatus.OTHERS && searchValue.status !== SearchStatus.USER_PLUGINS) && (
                        <div className={cl("grid")}>
                            {fcordPlugins}
                            {othersVisible}
                        </div>
                    ...
                    {fcordData.length === 0 && othersData.length === 0 && searchValue.status !== SearchStatus.USER_PLUGINS && (
                        <ExcludedPluginsList search={search} />
                    )}
