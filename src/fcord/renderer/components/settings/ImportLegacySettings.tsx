...
import { BaseText } from "@Fcord/types/components";
import { React, useRef,useState } from "react";

import { cl } from "./Settings";

function detectSource(json: any): "equicord" | "vencord" | "fcord" | "unknown" {
    ...
    // Plugins spécifiques à Fcord
    if (pluginNames.some(p => ["FcordHelper", "equicordHelper"].includes(p))) return "fcord";
...
function cleanForFcord(json: any): any {
    ...
    // Supprimer les plugins propres à Equicord/Vencord qui n'existent pas dans Fcord
    ...
    async function processFile(file: File) {
        ...
        try {
            ...
            const sourceLabel =
                ...
                source === "fcord" ? "Fcord" : "inconnu";

            const cleaned = cleanForFcord(json);
            ...
            setStatus("success");
            setMessage(`✅ Settings ${sourceLabel} importés avec succès ! Redémarre Fcord pour appliquer.`);
        } catch (err: any) {
            setStatus("error");
    ...
    return (
        <div className={cl("category")}>
            ...
            <BaseText size="sm" style={{ marginBottom: "12px", opacity: 0.7 }}>
                Drag & drop your Equicord or Vencord backup JSON file here to import your settings into Fcord.
                Plugin-specific settings that don't exist in Fcord will be automatically removed.
            </BaseText>
