...
const FcordDir = dirname(process.execPath);

export const PORTABLE =
    process.platform === "win32" &&
    !process.execPath.toLowerCase().endsWith("electron.exe") &&
    !existsSync(join(FcordDir, "Uninstall Fcord.exe"));

export const DATA_DIR =
    process.env.Fcord_USER_DATA_DIR || (PORTABLE ? join(FcordDir, "Data") : join(app.getPath("userData")));
...
export const USER_AGENT = `Fcord/${app.getVersion()} (https://git.${domain}/fcord/fcord)`;
