...
handleSync(IpcEvents.GET_GIT_HASH, () => Fcord_GIT_HASH);
...
// Handler pour VencordNative.fcord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.FcordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { FcordDir } = State.store;
    if (!FcordDir) return;

    const stats = await stat(FcordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(FcordDir);
});
